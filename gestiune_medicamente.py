...
def adauga_sau_suplimenteaza_medicament():
    ...
    concentratie = input("Concentrație (ex: 100mg, 200mg, 1g): ").strip()

    print("Forma de prezentare (Alege cifră: 1-Fiolă, 2-Tabletă, 3-Capsulă, 4-Sirop): ")
    opt_forma = input("> ").strip()
    forme_dict = {"1": "Fiolă", "2": "Tabletă", "3": "Capsulă", "4": "Sirop"}
    forma = forme_dict.get(opt_forma, "Altceva")

    # Verificăm dacă există deja exact această combinație de nume + concentrație + formă
    ...
    if medicament_existent:
        ...
        print(
            f"✅ Medicament existent detectat! Stocul pentru {medicament_existent['denumire']} {medicament_existent['concentratie']} a fost suplimentat la {medicament_existent['cantitate']} buc."
        )
    else:
        print("\n--- Atribute medicale noi sesizate ---")
        substanta = input("Substanța activă (ex: Amoxicilină): ").strip()
...
def consuma_medicament():
    ...
    if not med_gasit:
        print("❌ Medicamentul cu această concentrație nu a fost găsit în inventar.")
        return

    # Verificare securitate regim special
    if med_gasit.get("regim_special", False):
        print("⚠️ ATENȚIE: Acesta este un medicament cu REGIM SPECIAL (Narcotic/Psihotrop)!")
        parola = input("Introdu codul de autorizare medic/farmacist: ")
        if parola != "HMS123":  # O simulare simplă de verificare securitate
    ...
    try:
        cant_necesara = int(input("Cantitate necesară: "))
        if cant_necesara > med_gasit["cantitate"]:
            print("❌ Stoc insuficient!")
            return
        med_gasit["cantitate"] -= cant_necesara
        print(f"✅ Eliberat cu succes! Stoc rămas: {med_gasit['cantitate']} {med_gasit['forma']}e.")
        salveaza_medicamente(lista_medicamente)
    except ValueError:
        print("❌ Introdu un număr valid.")
...
def meniu_stocuri():
    ...
    while True:
        ...
        print("3. Eliberare Medicament (Scădere Stoc)")
        print("4. Ieșire")
